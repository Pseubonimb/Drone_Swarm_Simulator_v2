...
Пошаговый перебор **одной оси в YAML**, остальные коэффициенты задаются **флагами лаунчера** (см. ниже). Родительские `--kp`, `--ki`, `--kd`, `--derivative-alpha` пробрасываются в каждый дочерний прогон батча, если соответствующий логический ключ **не** входит в текущий sweep (на этапе 3 **`--ki` не указывайте** — его задаёт YAML).
...
```text
run_index,session_stamp,rel_exp_dir,kp,ki,kd,derivative_alpha,angle_max_cd,d_star_m,epsilon_m,T_epsilon_s,IAE_m_s,overshoot_m,J,notes
1,2026-04-10_20-00-00,experiments/.../batch_snake_ofat_s1_kp_run_1,2,0,0,0,3000,2.0,0.15,,,,,kp low oscillation
```

## Порядок этапов (рекомендация)

Дефолты сценария: `--kp 1`, `--ki 0`, `--kd 0`, `--derivative-alpha 0` (см. `snake_pursuit.py`). `ANGLE_MAX`: `config/iris.parm` → **3000** (30°).
...
### Этап 3 — I на обеих осях (`pid.i_gain` → `--ki`)

Фиксируйте **kp\***, **kd\***; **`--ki` в команду не добавляйте** (его перебирает YAML).

```bash
python launch_simulation.py -s --kp <kp_star> --kd <kd_star> --batch-params \
  scenarios/batch_parameters/ofat_snake_pursuit/stage3_ki.yaml
```

### Этап 4 — сглаживание D (`pid.derivative_alpha`)

```bash
python launch_simulation.py -s --kp <kp_star> --kd <kd_star> --ki <ki_star> --batch-params \
...
| `stage3_ki.yaml` | sweep `pid.i_gain` |
